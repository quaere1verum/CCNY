...
; Remember that the College is closed on Thursday
...
; Solution to Homework 2, Exercise 3
...
; here:  a is the (n + 1 - count)th fibonacci number
...
; count up rather than down? Try this for homework! )



; observe that the second version of fib is a linear iteration
...
; Leading up to Section 1.3.1 in Abelson and Sussman - two somewhat more involved proof examples
...
; a <= b AND result-so-far = sum of all integers i from A up to b, A <= i <= b, where A is the
...
; is that all we know on termination about a is that it is greater than b.  It could
; be b+1, or b+2, or ...
...
; not wanting to modify the precondition, we opt instead to modify the code
...
; sequence illustrates a key point about code development in this setting: one uses the logic
; side-by-side with the code in a mutually supportive refinement process.  We view programs as
; fluid systems, with deeply intertwined code and logic components.  


; you should work out arguments for the last two versions of sigma!  practice, practice, practice!
